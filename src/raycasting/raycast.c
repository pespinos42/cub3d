...
void	per_wall_dist(t_ray *r)
{
    ...
    // Verificar si perpWallDist es cero para evitar dividir por cero
    if (r->perp_wall_dist == 0)
    {
        // Manejar el caso cuando perpWallDist es cero
        // Puedes asignar un valor predeterminado o tomar alguna otra acción
        r->line_height = 0; // Establecer un valor predeterminado
    }
    else
    {
        // Calcular la altura de la línea normalmente
